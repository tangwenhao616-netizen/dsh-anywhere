...
export class SshFsCore {
   ...
   *   login/sshArgs 构造一条共享 ControlMaster 连接(fs 与 subprocess 可共用同一 conn);
   *   cwd: 相对路径基准。
   */
  constructor(config) {
    this.cwd = config.cwd ?? '/'
  ...
  /** 在远程跑一段 bash 脚本(经共享 ControlMaster 连接);返回 {code, stdout:Buffer, stderr}。 */
  _ssh(script, signal) { return this.conn.run(script, signal) }

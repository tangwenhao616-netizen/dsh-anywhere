...
# 5) 计划任务保活(开机触发)。-i 路径用引号拼接(可能含空格);端口用拼接避免 $Port: 被当驱动器
$tunnelArgs = '-N -T -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=accept-new -o IdentitiesOnly=yes -o BatchMode=yes -i "' + $Key + '" -R 127.0.0.1:' + $Port + ':localhost:22 -p ' + $RelayPort + ' ' + $RelayUser + '@' + $RelayHost

...
EOF
  ...
  RUN_CMD="ssh -N -T -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=accept-new -o IdentitiesOnly=yes -o BatchMode=yes -i \${KEY} -R 127.0.0.1:\${PORT}:localhost:22 -p \${RELAY_PORT} \${RELAY_USER}@\${RELAY_HOST}"
fi
if [ "\${OS}" = mac ]; then

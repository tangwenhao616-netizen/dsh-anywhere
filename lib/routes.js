...
export function makeRoutes(deps) {
  ...
  const frpDir = deps.frpDir ?? join(homedir(), '.dsh', 'fleet', 'frp')   // dph 托管 frpc 二进制处
  ...
  return [
    ...
    { kind: 'exact', path: '/api/fleet/revoke', handler: async (req, res) => {
      ...
      if (m) {
        // frp 机器:中继上没有该机的隧道授权键(frpc 用共享 token),故跳过 legacy 清理。
        // 注:frp 无法远程停机器上的 frpc(够不到),但移除主机条目 + 档案后 hub 不再连它;
        // 端口仅暴露该机自身 sshd,须持操作密钥方能进,故遗留 frpc 无安全影响。
        if (m.transport !== 'frp') { try { await provisioner.removeTunnelKey(m.port) } catch { /* 中继不可达也要继续本地清理 */ } }
    ...
    { kind: 'exact', path: '/api/fleet/relay', handler: async (req, res) => {
      ...
      store.update(f => {
        f.relay = {
          ...
          // frp 传输(可选;配了 frpToken 就走 frp,否则 legacy ssh -R)。未带则保留现值。
          frpToken: typeof body.frpToken === 'string' ? body.frpToken.trim() : (f.relay?.frpToken ?? ''),
          frpPort: Number.isInteger(body.frpPort) ? body.frpPort : (f.relay?.frpPort ?? 443),
          frpProtocol: typeof body.frpProtocol === 'string' && body.frpProtocol ? body.frpProtocol : (f.relay?.frpProtocol ?? 'tcp'),
        }
      })
    ...
    // ---- 公开:下载 frpc 二进制(入网机器 bootstrap 用;?os=win → frpc.exe)。目前仅 amd64。
    { kind: 'exact', path: '/api/fleet/frpc', handler: async (req, res) => {
      if (!fromTunnelOrLocal(req)) return writeJson(res, 403, { error: 'forbidden' })
      const url = new URL(req.url ?? '/', 'http://localhost')
      const name = url.searchParams.get('os') === 'win' ? 'frpc.exe' : 'frpc'
      const file = join(frpDir, name)
      if (!existsSync(file)) return writeJson(res, 404, { error: 'frpc binary not bundled on hub' })
      const buf = readFileSync(file)   // 命中稀少(仅入网时);二进制 ~15MB,一次性读入可接受
      res.writeHead(200, {

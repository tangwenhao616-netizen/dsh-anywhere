...
export class SshProvisioner {
  ...
  /**
   * 忘掉 hub known_hosts 里该端口的机器指纹(真机踩坑:机器重装系统后重入网,
   * host key 变了,hub 的 accept-new 因 changed 拒连且报错难懂)。重入网即换机语义,
   * 清旧指纹是预期行为;best-effort(条目不存在 ssh-keygen 也返回 0)。
   */
  async forgetHostKey(port) {
    try { await run('ssh-keygen', ['-R', `[127.0.0.1]:${port}`]) } catch { /* best effort */ }
  }
}

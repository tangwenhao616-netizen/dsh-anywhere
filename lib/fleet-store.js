 ...
 * frpToken 非空 → 入网走 frp(frpc↔frps),否则回退 legacy 反向 ssh -R。
 *   frpPort:frps 监听端口(默认 443,看着像 HTTPS,穿透性最好)。
 *   frpProtocol:frpc 传输(tcp 默认,控制连接自带 TLS;某些网络需换 kcp)。
 */
export const DEFAULT_RELAY = {
  host: '', port: 22, tunnelUser: 'tunnel',
  jumpAlias: 'relay-jump', jumpLogin: '', portRange: [20001, 20999],
  frpToken: '', frpPort: 443, frpProtocol: 'tcp',
}

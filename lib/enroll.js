...
export async function enrollCore(deps, req, now) {
  ...
  // 同名机器重入网**复用原端口**(真机踩坑:原实现分配时旧条目还在列表里,导致每次
  // 重入网端口漂移 20001→20003→…,授权行/主机条目/机器侧配置三方错位)。全新机器才分配。
  const prev = file.machines.find(m => m.alias === alias)
  const others = file.machines.filter(m => m.alias !== alias)
  const port = prev?.port ?? allocatePort(others.map(m => m.port), file.relay.portRange)
  ...
  // 重入网 = 机器可能重装过(host key 变化)→ 清 hub 上该端口旧指纹,避免 accept-new 拒连
  if (provisioner.forgetHostKey) await provisioner.forgetHostKey(port)

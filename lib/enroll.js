...
export async function enrollCore(deps, req, now) {
  ...
  // 传输层:frpToken 已配 → frp(机器 frpc 用共享 token 认到 frps,无需在中继授权隧道密钥);
  // 否则 legacy——把机器隧道公钥推到中继(permitlisten 锁端口)供反向 ssh -R。
  const useFrp = !!relay.frpToken
  if (!useFrp) await provisioner.pushTunnelKey(req.tunnelPublicKey, port)

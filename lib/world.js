 ...
 *   fs       —— 默认 true。false = 不注册 ctx.fs(SSHFS 挂载架构:文件走 hub 本地 fs
 *               经挂载点,ssh-world 只供 subprocess 远程执行)。
 *   pathMap  —— {from, to}:subprocess workdir 前缀映射(hub 挂载点 → 远程真实路径)。
 */
 ...
 * @param {{login:string, sshArgs?:string[], cwd?:string, platform?:'posix'|'windows', fs?:boolean, pathMap?:{from:string,to:string}}} [config]
 */
export function apply(ctx, config = {}) {
  ...
  if (config.fs !== false) ctx.plugin(SshFileSystem, { conn, cwd, platform })
  ctx.plugin(SshSubprocess, { conn, platform, pathMap: config.pathMap })
}
